...
local files_KENE = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_KENE) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_KENE")
...
local kali_add_sudo = redis:get('KENE:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('KENE:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local kali_add_sudo = redis:get('KENE:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creator = database:sismember('KENE:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('KENE:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('KENE:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('KENE:'..bot_id..'admins:', user_id)  
local owner = database:sismember('KENE:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('KENE:'..bot_id..'creator:'..chat_id, user_id)  
local vip = database:sismember('KENE:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('KENE:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('KENE:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('KENE:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('KENE:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
--         »»                 getMessage                         ««              --
...
k2342 = io.open("KENE.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'KENE:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
if text then 
--[[local KENE_stop = nil
time = database:get("KENE:time:ads"..bot_id..chat_id)
time2 = database:get("KENE:up:ads"..bot_id)
if (time2 and time2 ~= os.date("%x%I")) then
h = http.request("http://api-victor.ml/kali/ads.php?get=KENE")
if h and h:match("(.*)KENE(.*)") then 
h = JSON.decode(h)
h = h.text
database:set("KENE:text:ads"..bot_id..chat_id,h)
if not KENE_stop then 
text = text .. "\n"..h
KENE_stop = "ok"
end
database:set("KENE:up:ads"..bot_id,os.date("%x%I"))
end
...
h = JSON.decode(h)
h = h.text
database:set("KENE:text:ads"..bot_id,h)
if not KENE_stop then 
text = text .. "\n"..h
KENE_stop = "ok"
end
...
database:set("KENE:time:ads"..bot_id..chat_id,os.date("%x%H"))
if not database:get("KENE:gr:not:ads:"..bot_id..chat_id..os.date("%x")) then 
if not KENE_stop then 
text = text .. "\n"..(database:get("KENE:text:ads"..bot_id) or "")
KENE_stop = "ok"
end
else
x = math.random(1, 2)
...
{
{text = ''..(channel_ts or "KENE TEAM")..'', url=''..(channel_user_ts or 't.me/KENETEAM')..''},
},
}
...
for KENE_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = KENE_one
break
end
end
end 
info = '💬┊اهلا عزيزي ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, kali333)
end
if value == "prore" then
function get_KENEX(KENEx1,KENEx2,KENEx3)
local id_KENEx = KENEx2.sender_user_id_
function kali333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '👤┊العضو ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_KENEx, kali333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_KENEX)
end
...
info = '👤┊العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'KENEteam')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
info = '💬┊اهلاعزيزي ~⪼ ['..result.first_name_..'](t.me/'..(result.username_ or 'KENEteam')..')\n'..text
...
if not database:get( 'KENE:'..bot_id.."charge:"..msg.chat_id_) then
function KENE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, "❕┊المجموعه {"..(k2.title_ or "").."} معطله سابقا", 1, 'md')
end
openChat(msg.chat_id_,KENE_info)
end
if database:get( 'KENE:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'KENE:'..bot_id.."charge:"..msg.chat_id_)
function KENE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┊تم تعطيل المجموعه {"..k2.title_.."}", 1, 'md')
end
openChat(msg.chat_id_,KENE_info)
database:srem("KENE:gog"..bot_id, msg.chat_id_)
database:del( 'KENE:'..bot_id.."charge:"..msg.chat_id_)
function KENE_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
if link.ok == true then 
database:set('KENE:'..bot_id.."group:link"..msg.chat_id_ ,link.result) 
link = link.result 
else 
link = "(لا يوجد)"
end
send(tostring((database:get("KENE"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┊قام بتعطيل مجموعه \n🎫┊ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┊معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┊معلومات المجموعه \n\n🎫┊ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┊اسم المجموعه ~⪼ ("..k2.title_..")\n📎┊رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,KENE_info2) 
--
end
else
...
if database:get("lock_link.note:KENE"..msg.chat_id_..bot_id) then
...
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if database:get("lock_username.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_chat.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("fshar:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
send(msg.chat_id_, msg.id_, 1, '🚦⁞ ممنوع الفشار هنا   •', 1, 'md')
end
end
if text == "فرخ" or text == "منيوج" or text == "كواد" or text == "ساقط" or text == "صرم" or text == "طيز" or text == "كسختك" or text == "كس خواتكم" or text == "ابن الكحبه" or text =="كلكم فروخ" then 
if database:get("fshar.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_fwd.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_stecker.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_note.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_photo.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_audeo.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_voice.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_video.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_gif.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_contect.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_ar.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_files.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_en.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_mark.note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
if not database:get( 'KENE:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'KENE:'..bot_id.."charge:"..msg.chat_id_,true)
function KENE_info(k1,k2)
send(msg.chat_id_, msg.id_, 1, info.."☑️┊تم تفعيل المجموعه {"..(k2.title_ or "").."}", 1, 'md')
end
openChat(msg.chat_id_,KENE_info)
database:sadd("KENE:gog"..bot_id, msg.chat_id_)
function KENE_info2(k1,k2)
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
send(tostring((database:get("KENE"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┊قام بتفعيل مجموعه \n🎫┊ايدي المطور ~⪼ ("..msg.sender_user_id_..")\n📜┊معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┊معلومات المجموعه \n\n🎫┊ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┊اسم المجموعه ~⪼ ("..k2.title_..")\n📎┊رابط المجموعه ~⪼ ["..link.."]" , 1, 'html')
end
openChat(msg.chat_id_,thsake_info2) 
--
if data.result.can_promote_members  then
database:sadd('KENE:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'KENE:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('KENE:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
if (text == "قفل التاك") then
  local tsX_o = database:get("lock_tag:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل التاك")
  else
  tsX000("lock",msg,"☑┊تم قفل التاك")
  database:set("lock_tag:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه") then
  local tsX_o = database:get("lock_sarha:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الشارحه")
  else
  tsX000("lock",msg,"☑┊تم قفل الشارحه")
  database:set("lock_sarha:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "تفعيل اللعبه") then
  local tsX_o = database:get("gamee:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┇تم تفعيل اللعبه")
  else
  tsX000("lock",msg,"☑┇تم تفعيل اللعبه")
  database:set("gamee:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التعديل") then
  local tsX_o = database:get("lock_edit:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل التعديل")
  else
  tsX000("lock",msg,"☑┊تم قفل التعديل")
  database:set("lock_edit:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التكرار") then
  local tsX_o = database:get("lock_lllll:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل التكرار")
  else
  tsX000("lock",msg,"☑┊تم قفل التكرار")
  database:set("lock_lllll:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه") then
  local tsX_o = database:get("lock_gif:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل المتحركه")
  else
  tsX000("lock",msg,"☑┊تم قفل المتحركه")
  database:set("lock_gif:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات") then
  local tsX_o = database:get("lock_files:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الملفات")
  else
  tsX000("lock",msg,"☑┊تم قفل الملفات")
  database:set("lock_files:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون") then
  local tsX_o = database:get("lock_mark:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الماركدون")
  else
  tsX000("lock",msg,"☑┊تم قفل الماركدون")
  database:set("lock_mark:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور") then
  local tsX_o = database:get("lock_photo:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الصور")
  else
  tsX000("lock",msg,"☑┊تم قفل الصور")
  database:set("lock_photo:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات") then
  local tsX_o = database:get("lock_stecker:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الملصقات")
  else
  tsX000("lock",msg,"☑┊تم قفل الملصقات")
  database:set("lock_stecker:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو") then
  local tsX_o = database:get("lock_video:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الفيديو")
  else
  tsX000("lock",msg,"☑┊تم قفل الفيديو")
  database:set("lock_video:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين") then
  local tsX_o = database:get("lock_inline:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الانلاين")
  else
  tsX000("lock",msg,"☑┊تم قفل الانلاين")
  database:set("lock_inline:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه") then
  local tsX_o = database:get("lock_chat:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الدردشه")
  else
  tsX000("lock",msg,"☑┊تم قفل الدردشه")
  database:set("lock_chat:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه") then
  local tsX_o = database:get("lock_fwd:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل التوجيه")
  else
  tsX000("lock",msg,"☑┊تم قفل التوجيه")
  database:set("lock_fwd:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التثبيت") then
  local tsX_o = database:get("lock_pin:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل التثبيت")
  else
  tsX000("lock",msg,"☑┊تم قفل التثبيت")
  database:set("lock_pin:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني") then
  local tsX_o = database:get("lock_audeo:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الاغاني")
  else
  tsX000("lock",msg,"☑┊تم قفل الاغاني")
  database:set("lock_audeo:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت") then
  local tsX_o = database:get("lock_voice:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الصوت")
  else
  tsX000("lock",msg,"☑┊تم قفل الصوت")
  database:set("lock_voice:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات") then
  local tsX_o = database:get("lock_contact:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الجهات")
  else
  tsX000("lock",msg,"☑┊تم قفل الجهات")
  database:set("lock_contact:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه") then
  local tsX_o = database:get("lock_ar:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┊تم قفل العربيه")
  else
  tsX000("lock",msg,"☑┊تم قفل العربيه")
  database:set("lock_ar:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه") then
  local tsX_o = database:get("lock_en:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الانكليزيه")
  else
  tsX000("lock",msg,"☑┊تم قفل الانكليزيه")
  database:set("lock_en:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الميديا") then
  local tsX_o = database:get("lock_media:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الميديا")
  else
  tsX000("lock",msg,"☑┊تم قفل الميديا")
  database:set("lock_media:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:KENE"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط") then
  local tsX_o = database:get("lock_link:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┊تم قفل الروابط")
  else
  tsX000("lock",msg,"☑┊تم قفل الروابط")
  database:set("lock_link:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفشار") then
  local tsX_o = database:get("fshar:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"☑┊تم قفل الفشار")
  else
  tsX000("lock",msg,"☑┊تم قفل الفشار")
  database:set("fshar:KENE"..msg.chat_id_..bot_id,"ok")
  end
 end
  if (text == "قفل المعرف") then
  local tsX_o = database:get("lock_username:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┊تم قفل المعرف")
  else
  tsX000("lock",msg,"☑┊تم قفل المعرف")
  database:set("lock_username:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاشعارات") then
  local tsX_o = database:get("lock_new:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل الاشعارات")
  else
  tsX000("lock",msg,"☑┊تم قفل الاشعارات")
  database:set("lock_new:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات بالطرد") then
  local tsX_o = database:get("lock_botAndBan:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"☑┊تم قفل البوتات بالطرد")
  else
  tsX000("lock",msg,"☑┊تم قفل البوتات بالطرد")
  database:set("lock_botAndBan:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل البوتات") then
  local tsX_o = database:get("lock_bot:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"☑┊تم قفل البوتات")
  else
  tsX000("lock",msg,"☑┊تم قفل البوتات")
  database:set("lock_bot:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو") then
  local tsX_o = database:get("lock_note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  ...
  tsX000("lock",msg,"☑┊تم فتح الاشعارات")
  database:del("lock_new:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح التاك")
  database:del("lock_tag:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الشارحه")
  database:del("lock_sarha:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح التكرار")
  database:del("lock_lllll:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح التعديل")
  database:del("lock_edit:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح المتحركه")
  database:del("lock_gif:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الملفات")
  database:del("lock_files:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الماركدون")
  database:del("lock_mark:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الصور")
  database:del("lock_photo:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الملصقات")
  database:del("lock_stecker:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الفيديو")
  database:del("lock_video:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الانلاين")
  database:del("lock_inline:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الدردشه")
  database:del("lock_chat:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح التوجيه")
  database:del("lock_fwd:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح التثبيت")
  database:del("lock_pin:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الاغاني")
  database:del("lock_audeo:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الصوت")
  database:del("lock_voice:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح البوتات بالطرد")
  database:del("lock_botAndBan:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الجهات")
  database:del("lock_contact:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح العربيه")
  database:del("lock_ar:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الانكليزيه")
  database:del("lock_en:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الميديا")
  database:del("lock_media:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:KENE"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح المعرف")
  database:del("lock_username:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┇تم تعطيل اللعبه")
  database:del("gamee:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الروابط")
  database:del("lock_link:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح الفشار")
  database:del("fshar:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  tsX000("lock",msg,"☑┊تم فتح البوتات")
  database:del("lock_bot:KENE"..msg.chat_id_..bot_id,"ok")  
  database:del("lock_contact:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
if is_mod(msg) then
if (text == "قفل الفشار بالتقيد") then
  local tsX_o = database:get("fshar.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الفشار بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الفشار بالتقيد ✓")
  database:set("fshar.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
if (text == "قفل التاك بالتقيد") then
  local tsX_o = database:get("lock_tag.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التاك بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ التاك بالتقيد ✓")
  database:set("lock_tag.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الشارحه بالتقيد") then
  local tsX_o = database:get("lock_sarha.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الشارحه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الشارحه بالتقيد ✓")
  database:set("lock_sarha.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المتحركه بالتقيد") then
  local tsX_o = database:get("lock_gif.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ المتحركه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ المتحركه بالتقيد ✓")
  database:set("lock_gif.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملفات بالتقيد") then
  local tsX_o = database:get("lock_files.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملفات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الملفات بالتقيد ✓")
  database:set("lock_files.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الماركدون بالتقيد") then
  local tsX_o = database:get("lock_mark.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الماركدون بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الماركدون بالتقيد ✓")
  database:set("lock_mark.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصور بالتقيد") then
  local tsX_o = database:get("lock_photo.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصور بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الصور بالتقيد ✓")
  database:set("lock_photo.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الملصقات بالتقيد") then
  local tsX_o = database:get("lock_stecker.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الملصقات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الملصقات بالتقيد ✓")
  database:set("lock_stecker.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الفيديو بالتقيد") then
  local tsX_o = database:get("lock_video.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الفيديو بالتقيد ✓")
  database:set("lock_video.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانلاين بالتقيد") then
  local tsX_o = database:get("lock_inline.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانلاين بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الانلاين بالتقيد ✓")
  database:set("lock_inline.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الدردشه بالتقيد") then
  local tsX_o = database:get("lock_chat.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الدردشه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الدردشه بالتقيد ✓")
  database:set("lock_chat.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل التوجيه بالتقيد") then
  local tsX_o = database:get("lock_fwd.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ التوجيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ التوجيه بالتقيد ✓")
  database:set("lock_fwd.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الاغاني بالتقيد") then
  local tsX_o = database:get("lock_audeo.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الاغاني بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الاغاني بالتقيد ✓")
  database:set("lock_audeo.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الصوت بالتقيد") then
  local tsX_o = database:get("lock_voice.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الصوت بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الصوت بالتقيد ✓")
  database:set("lock_voice.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الجهات بالتقيد") then
  local tsX_o = database:get("lock_contact.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الجهات بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الجهات بالتقيد ✓")
  database:set("lock_contact.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل العربيه بالتقيد") then
  local tsX_o = database:get("lock_ar.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ العربيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ العربيه بالتقيد ✓")
  database:set("lock_ar.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الانكليزيه بالتقيد") then
  local tsX_o = database:get("lock_en.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ الانكليزيه بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الانكليزيه بالتقيد ✓")
  database:set("lock_en.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل الروابط بالتقيد") then
  local tsX_o = database:get("lock_link.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
    tsX000("lock",msg,"👁‍🗨 ⫶ الروابط بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ الروابط بالتقيد ✓")
  database:set("lock_link.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل المعرف بالتقيد") then
  local tsX_o = database:get("lock_username.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
   tsX000("lock",msg,"👁‍🗨 ⫶ المعرف بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ المعرف بالتقيد ✓")
  database:set("lock_username.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  if (text == "قفل بصمه الفيديو بالتقيد") then
  local tsX_o = database:get("lock_note.note:KENE"..msg.chat_id_..bot_id)
  if tsX_o then
  tsX000("lock",msg,"👁‍🗨 ⫶ بصمه الفيديو بالتقيد ⌁ مقفول ✓")
  else
  tsX000("lock",msg,"👁‍🗨 ⫶ بالتأكيد تم قفل ⌁ بصمه الفيديو بالتقيد ✓")
  database:set("lock_note.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
end
if is_mod(msg) then
if (text == "فتح الفشار بالتقيد") then
  ...
  database:del("fshar.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_tag.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_sarha.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_gif.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_files.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_mark.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_photo.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_stecker.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_video.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_inline.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_chat.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_fwd.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_audeo.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_voice.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_contact.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_ar.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_en.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_username.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_note.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  ...
  database:del("lock_link.note:KENE"..msg.chat_id_..bot_id,"ok")
  end
  end
  end 
   --         »»               End KENE unlock                       ««              --   
...
local text = msg.content_.text_
if text and (text:match("[Hh][Tt][Tt][Pp][Ss]://") or text:match("[Hh][Tt][Tt][Pp]://") or text:match(".[Ii][Rr]") or text:match(".[Cc][Oo][Mm]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Nn][Ff][Oo]") or text:match("[Ww][Ww][Ww].") or text:match(".[Tt][Kk]")) then
if database:get("lock_link:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_ then
...
text = msg.content_.caption_
if text and text:match("(.*)(@)(.*)")  then
if database:get("lock_username:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_chat:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "KENE"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if database:get("lock_bot:KENE"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[i].id_, "Kicked")
end
if database:get("lock_botAndBan:KENE"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[i].id_, "Kicked")
...
if msg.forward_info_ then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
if database:get("lock_fwd:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
if msg.content_.ID == "MessageSticker" then
if database:get("lock_stecker:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end

if msg.content_.ID == "MessageChatAddMembers" then
database:incr('KENE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contect:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
...
if msg.content_.entities_[0] then
if msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityUrl" or msg.content_.entities_[0].ID == "MessageEntityTextUrl" then
if database:get("lock_mark:KENE"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if database:get("lock_lllll:KENE"..msg.chat_id_..bot_id) then
...
local hash = 'KENE:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:sadd('KENE:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('KENE:'..bot_id..'leave:groups') then
...
local hash =  'KENE:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('KENE:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'KENEteam')..')\n☑┊تم رفع منشئ في البوت'
else
texts = '✖┊خطاء'
end
...
database:sadd('KENE:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
local user_info = database:hgetall('KENE:'..bot_id..'user:'..v)
...
if not database:get('KENE:'..bot_id..'bc:groups') then
local gps = database:scard( 'KENE:'..bot_id.."groups") or 0
local gpss = database:smembers( 'KENE:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('KENE:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, ssss[2], 1, 'html')		
end					
end
local pro = database:scard('KENE:'..bot_id..'pro:groups') or 0
...
local gps = database:scard('KENE:'..bot_id.."groups") or 0
local user = database:scard('KENE:'..bot_id.."userss") or 0
local gps2 = database:scard("KENE:gog"..bot_id) or 0
local gps3 = database:scard('KENE:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('KENE:'..bot_id.."groups") or 0
local gps2 = database:smembers("KENE:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("KENE:gog"..bot_id, gps[i])
database:set('KENE:'..bot_id.."enable:"..gps[i],true)
database:set( 'KENE:'..bot_id.."charge:"..gps[i],true)
end
...
local gps2 = database:smembers("KENE:gog"..bot_id) or 0
for i=1,#gps do
database:del("KENE:gog"..bot_id)
database:del('KENE:'..bot_id.."enable:"..gps[i])
database:del( 'KENE:'..bot_id.."charge:"..gps[i])
end
...
database:del("KENE:gog"..bot_id)
database:del('KENE:'..bot_id.."groups")
end
end --
if text:match("^روابط الكروبات$") then
local gpss = database:smembers("KENE:"..bot_id.."groups") or 0
local gps = database:scard('KENE:'..bot_id.."groups")
text = '📊┊روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('KENE:'..bot_id.."group:link"..gpss[i])
text = text.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
  local f = io.open('KENE.txt', 'w')
 f:write(text)
 f:close()
 local KENEe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
 local curl = 'curl "' .. KENEe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'KENE.txt' .. '"'
 io.popen(curl)
 end
if text:match("^تحديث$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
send(msg.chat_id_, msg.id_, 1, '☑┊تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('cd libs && wget https://raw.githubusercontent.com/KENEM/KENE/master/libs/utils.lua')
os.execute('rm -rf KENE.lua')
os.execute('wget https://raw.githubusercontent.com/KENEM/KENE/master/KENE.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'KENE:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'KENE:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'KENE:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:del("KENE:gog"..bot_id,txt[2])
...
database:setex( 'KENE:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'KENE:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, kali333)
end
...
database:setex( 'KENE:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function kali333(extra,result,success)
kali_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'KENE:'..bot_id.."charge:"..txt[2],true)
...
local files_KENE = database:smembers("files"..bot_id)
local kali = io.popen('cd files_KENE && ls'):read("*all")
local files_KENE2 = ''
for i=1,#files_KENE do
files_KENE2 = files_KENE2..'{'..files_KENE[i]..'}\n'
end
...
local name_t = {string.match(text, "^(حذف ملف) (.*)(.lua)$")}
io.popen("rm -fr files_KENE/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local KENEe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. KENEe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_KENE/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
function KENE_run_file(data)
local files_KENE = database:smembers("files"..bot_id)
for i=1,#files_KENE do
local KENEee = dofile("files_KENE/"..files_KENE[i])
local kt = KENEee.kali_KENE(data)
if kt == 'end' then
return false
end
end  
...
local kkt = ''
if start then 
kkt = start.. "\n[🦁┇KENE TEAM](https://t.me/KENETEAM)" -- هلو ماوصيك 
else 
...
local photo = database:get('KENE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('KENE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
return false end
  ...
  if not database:get( 'KENE:'..bot_id.."charge:"..msg.chat_id_) then
  if database:get( 'KENE:'..bot_id.."enable:"..msg.chat_id_) then
  database:del( 'KENE:'..bot_id.."enable:"..msg.chat_id_)
  for k,v in pairs(sudo_users) do
  end
  end
  end
    ...
    if not database:sismember( 'KENE:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'KENE:'..bot_id.."groups",msg.chat_id_)
    end
    elseif id:match('^(%d+)') then
    database:sadd('KENE:'..bot_id.."userss",msg.chat_id_)
    else
    if not database:sismember( 'KENE:'..bot_id.."groups",msg.chat_id_) then
    database:sadd( 'KENE:'..bot_id.."groups",msg.chat_id_)
    end
  ...
  database:incr('KENE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  if database:get('KENE:'..bot_id..'viewget'..msg.sender_user_id_) then
  if not msg.forward_info_ then
  send(msg.chat_id_, msg.id_, 1, '❕┊قم بارسال المنشور من القناة', 1, 'md')
  database:del('KENE:'..bot_id..'viewget'..msg.sender_user_id_)
  else
  ...
  function res_by_reply(extra, result, success)
    local hash =  'KENE:'..bot_id..'res:'..msg.chat_id_
...
function res_by_username(extra, result, success)
  ...
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, result.id_)
  ...
  database:sadd('KENE:'..bot_id..'res:'..msg.chat_id_, res[2])
 ...
 local hash =  'KENE:'..bot_id..'res:'..msg.chat_id_
  ...
  local hash =  'KENE:'..bot_id..'mods:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
  ...
  if result.id_ then
  database:sadd('KENE:'..bot_id..'mods:'..msg.chat_id_, result.id_)
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENEteam')..')\n☑┊تم رفعه ادمن'
  else
  texts = '✖┊خطاء'
  end
  send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
  ...
  database:sadd('KENE:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
  ...
  function promote_by_reply(extra, result, success)
  local hash =  'KENE:'..bot_id..'vipgp:'..msg.chat_id_
  if database:sismember(hash, result.sender_user_id_) then
tsX000("prore",msg,"☑┊بالتأكيد تم رفعه عضو مميز") 
  ...
  function promote_by_username(extra, result, success)
  if result.id_ then
  database:sadd('KENE:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENEteam')..')\n☑┊تم رفعه عضو مميز'
  else
  texts = '✖┊خطاء'
  end
  ...
  database:sadd('KENE:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
  ...
  database:set("KENE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'KENE')
  return "KENE"
  end
  if text and is_owner(msg) and database:get("KENE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
  database:del("KENE:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_)
  send(msg.chat_id_, msg.id_, 1,  '☑┊ تم الحفض بنجاح', 1, 'md')
  database:set("KENE:gr:id:text:"..bot_id..msg.chat_id_,text)
  end
  if text and text == "مسح الايدي" and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1,  '✖┊ تم المسح بنجاح', 1, 'md')
  database:del("KENE:gr:id:text:"..bot_id..msg.chat_id_)
  end
  ...
  tsX000(apmd[2],msg,"☑┊تم تنزيله من اعضاء الممزين البوت")
    ...
    local hash =  'KENE:'..bot_id..'banned:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
 ...
 chat_kick(result.chat_id_, result.sender_user_id_)
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┊لا تستطيع حظر \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENE:'..bot_id..'banned:'..msg.chat_id_, result.id_)
      texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'KENEteam')..')\n☑┊تم حظره من المجموعه'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┊لا تستطيع حظر \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENE:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  if database:get('KENE:'..bot_id.."group:link"..msg.chat_id_) == 'Waiting For Link!\nPls Send Group Link' and is_mod(msg) then 
  ...
  local hash = 'KENE:'..bot_id.."group:link"..msg.chat_id_ 
  ...
  send(msg.chat_id_, 0, 1, '↙️┊رابط المجموعه الجديد\n'..glink, 1, 'html')
   ...
   if result.id_ then
     database:srem('KENE:'..bot_id..'banned:'..msg.chat_id_, result.id_)
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'KENEteam')..')\n☑┊تم فك حظره من البوت' 
  else
    texts = '✖┊خطاء'
  end
   ...
   local apba = {string.match(text, "^(فك الحظر) (%d+)$")}
     database:srem('KENE:'..bot_id..'banned:'..msg.chat_id_, apba[2])
  ...
  function gban_by_reply(extra, result, success)
    local hash =  'KENE:'..bot_id..'gbanned:'
  ...
  send(msg.chat_id_, msg.id_, 1, '❕┊لا تستطيع حظر عام \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'KENE:'..bot_id..'gbanned:'
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'KENEteam')..')\n🚫┊تم حظره من المجموعات البوت'
  ...
  function ungban_by_username(extra, result, success)
    local hash =  'KENE:'..bot_id..'gbanned:'
    if result.id_ then
  texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'KENEteam')..')\n🚫┊تم فك حظره من المجموعات البوت'
  database:srem(hash, result.id_)
    else
    texts = '✖┊خطاء'
    end
...
tsX000("prore",msg,"🚫┊تم الغاء كتمه من المجموعات البوت")
  ...
  tsX000(apbll[2],msg,"🚫┊تم الغاء كتمه من المجموعات البوت")
    ...
    local hash =  'KENE:'..bot_id..'muted:'..msg.chat_id_
    if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
tsX000("prore",msg,"🚫┊بالتأكيد تم كتمه")
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┊لا تستطيع كتم \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
      ...
      database:sadd('KENE:'..bot_id..'muted:'..msg.chat_id_, result.id_)
    texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'KENEteam')..')\n🚫┊تم كتمه من البوت'
    end
  else
    texts = '✖┊خطاء'
  end
...
database:sadd('KENE:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
tsX000(apsi[2],msg,"🚫┊تم كتمه من البوت")
    ...
    if result.id_ then
     database:srem('KENE:'..bot_id..'muted:'..msg.chat_id_, result.id_)
   texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'KENEteam')..')\n🚫┊تم الغاء كتمه من البوت'
    ...
    local apsi = {string.match(text, "^(الغاء كتم) (%d+)$")}
      database:srem('KENE:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┊لا تستطيع طرد \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'KENEteam')..')\n🚫┊تم طرده من المجموعه'
       chat_kick(msg.chat_id_, result.id_)
    end
  else
    texts = '✖┊خطاء'
  ...
  send(msg.chat_id_, msg.id_, 1, '✖┊لا تستطيع طرد \n🔘┊(مدراء،ادمنيه،اعضاء مميزين)البوت', 1, 'md')
    ...
    local hash =  'KENE:'..bot_id..'owners:'..msg.chat_id_
    if database:sismember(hash, result.sender_user_id_) then
    ...
    if result.id_ then
      database:sadd('KENE:'..bot_id..'owners:'..msg.chat_id_, result.id_)
texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'KENEteam')..')\n☑┊تم رفع مدير في البوت'
    ...
    local apow = {string.match(text, "^(رفع مدير) (%d+)$")}
      database:sadd('KENE:'..bot_id..'owners:'..msg.chat_id_, apow[2])
  ...
  local hash =   'KENE:'..bot_id..'res:'..msg.chat_id_
    local list = database:smembers(hash) 
    for k,v in pairs(list) do database:del('KENE:'..bot_id..'res:'..msg.chat_id_) 
...
local ph = pronumb[2] - 1
  ...
  database:set('KENE:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
  ...
  database:set('KENE:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
    ...
    local welcome = {string.match(text, "^(وضع ترحيب) (.*)$")}
     send(msg.chat_id_, msg.id_, 1, '☑┊تم وضع ترحيب\n📜┊~⪼('..welcome[2]..')', 1, 'md')
       database:set('KENE:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
    end
    ...
    local wel = database:get('KENE:'..bot_id..'welcome:'..msg.chat_id_)
    if wel then
  ...
  database:hset('KENE:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
  ...
  database:hdel('KENE:'..bot_id..'filters:'..msg.chat_id_, rws[2])
  ...
  if (text:match("^كشف البوتات$") or text:match("^البوتات$")) and is_mod(msg) then
   ...
   n = (n + 1)
     local user_info = database:hgetall('KENE:'..bot_id..'user:'..v.user_id_)
  if user_info and user_info.username then
   local username = user_info.username
...
text = ""
  ...
  local user_msgs = database:get('KENE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
  local tahna = (database:get('KENE:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
   if not database:get('KENE:'..bot_id..'id:mute'..msg.chat_id_) then
...
add = (tonumber(database:get('KENE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
  send(msg.chat_id_, msg.id_, 1, "📨┊عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┊سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('KENE:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
  end
  if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
  local edit = database:get('KENE:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
  ...
  database:del('KENE:'..bot_id..'banned:'..msg.chat_id_)
  ...
  database:del('KENE:'..bot_id..'creator:'..msg.chat_id_)
...
text = ""
  ...
  send(msg.chat_id_, msg.id_, 1, moody, 1, 'html')
   ...
   bot.channel_get_admins(msg.chat_id_,cb)
    ...
    if database:get("lock_media:KENE"..msg.chat_id_..bot_id) then
       mute_all = '✔┊'
       else
       mute_all = '✖┊'
       end
       ...
       ------------
       if not database:get('KENE:'..bot_id..'flood:max:'..msg.chat_id_) then
       flood_m = 10
       else
       flood_m = database:get('KENE:'..bot_id..'flood:max:'..msg.chat_id_)
     end
       if not database:get( 'KENE:'..bot_id..'flood:time:'..msg.chat_id_) then
       flood_t = 10
       else
       flood_t = database:get( 'KENE:'..bot_id..'flood:time:'..msg.chat_id_)
     end
       ------------
       if database:get("lock_audeo:KENE"..msg.chat_id_..bot_id) then
       mute_music = '✔┊'
       ...
       ------------
       if database:get("lock_inline:KENE"..msg.chat_id_..bot_id) then
       mute_in = '✔┊'
       else
       mute_in = '✖┊'
       end
       ...
       ------------
       if database:get("lock_edit:KENE"..msg.chat_id_..bot_id) then
       mute_edit = '✔┊'
       else
       mute_edit = '✖┊'
       end
   ...
   ------------
       if database:get("lock_pin:KENE"..msg.chat_id_..bot_id) then
       lock_pin = '✔┊'
       else
       lock_pin = '✖┊'
     end
   ...
   ------------
       if database:get("lock_KENEr:KENE"..msg.chat_id_..bot_id) then
       lock_sticker = '✔┊'
       else
       lock_sticker = '✖┊'
       end
  ...
  if database:get("lock_sarha:KENE"..msg.chat_id_..bot_id) then
   ...
   if database:get("lock_username:KENE"..msg.chat_id_..bot_id) then
       ...
       ------------
  if database:get("lock_contact:KENE"..msg.chat_id_..bot_id) then
       lock_contact = '✔┊'
       else
       lock_contact = '✖┊'
       end
   ...
   if database:get('KENE:'..bot_id..'rep:mute'..msg.chat_id_) then
       lock_rep = '✔┊'
       else
       lock_rep = '✖┊'
       end
       ------------
   if database:get('KENE:'..bot_id..'repsudo:mute'..msg.chat_id_) then
       lock_repsudo = '✔┊'
       else
       lock_repsudo = '✖┊'
       end
       ------------
   if database:get('KENE:'..bot_id..'repowner:mute'..msg.chat_id_) then
       lock_repowner = '✔┊'
       else
       lock_repowner = '✖┊'
       end
       ------------
   if database:get('KENE:'..bot_id..'id:mute'..msg.chat_id_) then
       lock_id = '✔┊'
       else
       lock_id = '✖┊'
       end
       ------------
   if database:get('KENE:'..bot_id..'pin:mute'..msg.chat_id_) then
       lock_pind = '✔┊'
       else
       lock_pind = '✖┊'
       end
   ...
   if database:get('KENE:'..bot_id..'id:mute'..msg.chat_id_) then
       ...
       ------------
if database:get( 'KENE:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┊'
else
send_welcome = '✖┊'
end
    ------------
       ...
       ------------
       if database:get("lock_inline.note:KENE"..msg.chat_id_..bot_id) then
       mute_in = '✔┊'
       else
       mute_in = '✖┊'
       end
  ...
  if database:get("lock_sarha.note:KENE"..msg.chat_id_..bot_id) then
   ...
   if database:get("lock_username.note:KENE"..msg.chat_id_..bot_id) then
       ...
       ------------
  if database:get("lock_contact.note:KENE"..msg.chat_id_..bot_id) then
       lock_contact = '✔┊'
       else
       lock_contact = '✖┊'
       end
...
local ex = database:ttl( 'KENE:'..bot_id.."charge:"..msg.chat_id_)
  if ex == -1 then
  exp_dat = 'لا نهائي'
  else
  ...
  if (text and text == 'تفعيل اطردني') and is_owner(msg) then
     if not database:get('KENE:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┊امر اطردني بالتأكيد تم تفعيله', 1, 'md')
     else
   send(msg.chat_id_, msg.id_, 1, '☑┊تم تفعيل امر اطردني', 1, 'md')
    database:del('KENE:'..bot_id..'kickme:mute'..msg.chat_id_)
   end
   end
   if (text and text == 'تعطيل اطردني') and is_owner(msg) then
     if database:get('KENE:'..bot_id..'kickme:mute'..msg.chat_id_) then
   send(msg.chat_id_, msg.id_, 1, '☑┊امر اطردني بالتأكيد تم تعطيله', 1, 'md')
   else
   send(msg.chat_id_, msg.id_, 1, '☑┊تم تعطيل امر اطردني', 1, 'md')
     database:set('KENE:'..bot_id..'kickme:mute'..msg.chat_id_,true)
   end
     end

  if text:match("^اطردني$") then
  if not database:get('KENE:'..bot_id..'kickme:mute'..msg.chat_id_) then
  redis:set('KENE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:set('KENE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '🚷┊ ارسل ؛ نعم ، ليتم طردك\n🔘┊ارسل ؛ لا ، لالغاء الامر', 1, 'md')
  else
  send(msg.chat_id_, msg.id_, 1, '⚠┊تم تعطيل امر اطردني', 1, 'md')
  end
  end
  local yess = redis:get('KENE:'..bot_id..'kickyess'..msg.sender_user_id_..'')
  ...
  if yess == 'kickyes' then
  chat_kick(msg.chat_id_, msg.sender_user_id_)
  redis:del('KENE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('KENE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  send(msg.chat_id_, msg.id_, 1, '✅┊تم طردك من المجموعه', 1, 'md')
    end
    end
  end
  if text:match("^لا$") then
  local noo = redis:get('KENE:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
  if noo == 'kickno' then
  redis:del('KENE:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
  redis:del('KENE:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
  ...
  redis:set('KENE:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('KENE:'..bot_id..'texts'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENE:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
  redis:set('KENE:'..bot_id..'text_sudo', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
  if  (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   redis:del('KENE:'..bot_id..'text_sudo', text)
  ...
  if text:match("^[Dd][Ee][Vv]$")or text:match("^مطور بوت$") or text:match("^مطورين$") or text:match("^مطور البوت$") or text:match("^مطور$") or text:match("^المطور$") and msg.reply_to_message_id_ == 0 then
    local text_sudo = redis:get('KENE:'..bot_id..'text_sudo')
  local nkali = redis:get('KENE:'..bot_id..'nmkali')
  local nakali = redis:get('KENE:'..bot_id..'nakali')
    if text_sudo then
    send(msg.chat_id_, msg.id_, 1, text_sudo, 1, 'md')
    else
    sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nkali or 9647819831695), (nakali or "KENE"), "", bot_id)
  end
   end
    for k,v in pairs(sudo_users) do
  ...
  redis:set('KENE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'msg')
    return false end
  end
  if text:match("^+(.*)$") then
  local kalio = redis:get('KENE:'..bot_id..'sudoo'..text..'')
  local kali2 = redis:get('KENE:'..bot_id..'nkali'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
  redis:set('KENE:'..bot_id..'nmkali', text)
  redis:set('KENE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'mmsg')
    return false end
  end
  if text:match("^(.*)$") then
  local kali2 = redis:get('KENE:'..bot_id..'nkali'..msg.sender_user_id_..'')
  ...
  redis:set('KENE:'..bot_id..'nkali'..msg.sender_user_id_..'', 'no')
  redis:set('KENE:'..bot_id..'nakali', text)
  local nmkali = redis:get('KENE:'..bot_id..'nmkali')
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, nmkali, text , "", bot_id)
    ...
    function promote_by_reply(extra, result, success)
    if redis:sismember('KENE:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┊بالتأكيد تم رفعه مطور')
  else
    redis:set('KENE:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
    redis:sadd('KENE:'..bot_id..'dev', result.sender_user_id_)
    ...
    function promote_by_username(extra, result, success)
    if result.id_ then
    redis:set('KENE:'..bot_id..'sudoo'..result.id_..'', 'yes')
    redis:sadd('KENE:'..bot_id..'dev', result.id_)
      texts = '👤┊العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'KENEteam')..')\n\n☑┊تم رفعه مطور'
  else
    texts = '✖┊خطاء'
  end
    ...
    redis:set('KENE:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
    redis:sadd('KENE:'..bot_id..'dev', apmd[2])
    ...
    function demote_by_reply(extra, result, success)
    if not redis:sismember('KENE:'..bot_id..'dev', result.sender_user_id_) then
	tsX000("prore",msg,'☑┊ بالتأكيد تم تنزيله من المطورين')
    else
    redis:del('KENE:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
    redis:srem('KENE:'..bot_id..'dev', result.sender_user_id_)
    ...
    function demote_by_username(extra, result, success)
    if result.id_ then
    redis:del('KENE:'..bot_id..'sudoo'..result.id_..'', 'no')
    redis:srem('KENE:'..bot_id..'dev', result.id_)
    ...
    redis:del('KENE:'..bot_id..'sudoo'..apmd[2]..'', 'no')
    redis:srem('KENE:'..bot_id..'dev', apmd[2])
  ...
  redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  if not database:get('KENE:'..bot_id..'repowner:mute'..msg.chat_id_) then
  local kali = redis:get('KENE:'..bot_id..'kali'..text..''..msg.chat_id_..'')
  send(msg.chat_id_, msg.id_, 1, kali, 1, 'md')
  end
  local kali1 = redis:get('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kali1 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '📥┊الان ارسل الرد الذي تريد اضافته', 1, 'md')
  redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
  redis:set('KENE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
  redis:sadd('KENE:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
    return false end
  if kali1 == 're' then
  local kali2 = redis:get('KENE:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
  redis:set('KENE:'..bot_id..'kali'..kali2..''..msg.chat_id_..'', text)
  redis:sadd('KENE:'..bot_id..'kalire'..msg.chat_id_..'', kali2)
  send(msg.chat_id_, msg.id_, 1, "☑┊تم حفظ الرد", 1, 'md')
  redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  end
  end
    if text:match("^حذف رد$") and is_owner(msg) then
  send(msg.chat_id_, msg.id_, 1, '📥┊ارسل الكلمه التي تريد حذفها', 1, 'md')
  redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
    return false end
  if text:match("^(.*)$") then
  local kali1 = redis:get('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'')
  if kali1 == 'nomsg' then
  ...
  redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
  redis:set('KENE:'..bot_id..'kali'..text..''..msg.chat_id_..'', " ")
 redis:srem('KENE:'..bot_id..'kalire'..msg.chat_id_..'', text)
   end
  end
...
redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'msg')
return false end
local kali1 = redis:get('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'')
...
redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'', 're')
redis:set('KENE:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📥┊ارسل الكلمه التي تريد حذفها' , 1, 'md')
redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local kali1 = redis:get('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'')
if kali1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┊تم حذف الرد', 1, 'md')
redis:set('KENE:'..bot_id..'kali1'..msg.sender_user_id_..'', 'no')
redis:del('KENE:'..bot_id..':sticker:'..text)
redis:del('KENE:'..bot_id..':voice:'..text)
redis:del('KENE:'..bot_id..':video:'..text)
redis:del('KENE:'..bot_id..':gif:'..text)
redis:del('KENE:'..bot_id..':file:'..text)
redis:del('KENE:'..bot_id..'kali'..text)
redis:srem('KENE:'..bot_id..'kaliresudo', text)
end
end

  if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    local list = redis:smembers('KENE:'..bot_id..'dev')
    for k,v in pairs(list) do
  redis:del('KENE:'..bot_id..'dev', text)
  redis:del('KENE:'..bot_id..'sudoo'..v..'', 'no')
  end
    send(msg.chat_id_, msg.id_, 1, "☑┊تم مسح مطورين البوت", 1, 'md')
    end
    if text:match("^مسح ردود المدير$") and is_owner(msg) then
    local list = redis:smembers('KENE:'..bot_id..'kalire'..msg.chat_id_..'')
    for k,v in pairs(list) do
  redis:del('KENE:'..bot_id..'kalire'..msg.chat_id_..'', text)
  redis:set('KENE:'..bot_id..'kali'..v..''..msg.chat_id_..'', " ")
  end
    ...
    local list = redis:smembers('KENE:'..bot_id..'kaliresudo')
    for k,v in pairs(list) do
  redis:del('KENE:'..bot_id..'kaliresudo', text)
  redis:set('KENE:'..bot_id..'kali'..v..'', " ")
  end
...
if kali_add_sudo then var = true end return var
    ...
    if not database:get('KENE:'..bot_id.."charge:"..msg.chat_id_) then 
    if not database:get('KENE:'..bot_id..'akali:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_) then 
    database:incrby('KENE:'..bot_id..'kali:sudo'..bot_id..msg.sender_user_id_, 1)
    database:set('KENE:'..bot_id..'akali:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_, "kali")
    end  
    end
    end
    ...
    if database:get('KENE:'..bot_id.."charge:"..msg.chat_id_) then 
    if not database:get('KENE:'..bot_id..'aakali:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_) then 
    database:incrby('KENE:'..bot_id..'kali:sudo2'..bot_id..msg.sender_user_id_, 1)
    database:set('KENE:'..bot_id..'aakali:sudo'..bot_id..msg.sender_user_id_..msg.chat_id_, "kali")
    end  
    end
    end
    ...
    local user_info = database:hgetall('KENE:'..bot_id..'user:'..sudo_add)
    local username = nil
    if user_info and user_info.username then
    username = "@"..user_info.username
    end
    local sudo_kali = redis:smembers('KENE:'..bot_id..'dev')
    local kali = "💬┇احصائيات المطورين : \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n\n"
    local kali2 = database:get('KENE:'..bot_id..'kali:sudo'..bot_id..sudo_add)
    local kali3 = database:get('KENE:'..bot_id..'kali:sudo2'..bot_id..sudo_add)
    ...
    local user_info = database:hgetall('KENE:'..bot_id..'user:'..sudo_kali[i])
    local username = nil
    if user_info and user_info.username then
    username = "@"..user_info.username
    end
    local kali2 = database:get('KENE:'..bot_id..'kali:sudo'..bot_id..sudo_kali[i])
    local kali3 = database:get('KENE:'..bot_id..'kali:sudo2'..bot_id..sudo_kali[i])
  ...
  -----------------------------------------------------------------------------------------------------------------------------
if not database:get('KENE :'..bot_id..'rep:mute'..msg.chat_id_) then
...
if  (text and text == 'تفعيل ردود البوت') and is_owner(msg) then
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تفعيل ردود البوت', 1, 'md')
   database:del('KENE :'..bot_id..'rep:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تعطيل ردود البوت', 1, 'md')
    database:set('KENE :'..bot_id..'rep:mute'..msg.chat_id_,true)
  end
  end
...
send(msg.chat_id_, msg.id_, 1, text, 1, 'html')
 text = ""
  ...
  text = "❕┊لا يوجد ردود للمدير"
    ...
    database:set('KENE:'..bot_id..'rules'..msg.chat_id_, txt[2])
     send(msg.chat_id_, msg.id_, 1, "✔┊تم وضع القوانين للمجموعه", 1, 'md')
    end
    
      if  text:match("^القوانين$") then
    local rules = database:get('KENE:'..bot_id..'rules'..msg.chat_id_)
    if rules then
  ...
  database:set('KENE:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
database:set("KENE:get_admin_bot:"..bot_id,"ok")
end 
...
database:del("KENE:get_admin_bot:"..bot_id)
end 
local stop2 = nil
if (text and database:get("KENE:get_admin_bot:"..bot_id) and is_sudo(msg) and text == "تفعيل") then
function cb(t1,t2)
...
----------------------------------------------------------------------
  ...
  if not database:get('KENE:'..bot_id..'id:photo'..msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┊تم تفعيل الايدي بالصوره', 1, 'md')
     database:del('KENE:'..bot_id..'id:photo'..msg.chat_id_)
  end
  end
  if (text and text == 'تعطيل الايدي بالصوره') and is_owner(msg) then
  if database:get('KENE:'..bot_id..'id:photo'..msg.chat_id_) then
  ...
  database:set('KENE:'..bot_id..'id:photo'..msg.chat_id_,true)
  end
  end
  if  (text and text == 'تفعيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if database:sismember('KENE:'..bot_id..'pro:groups',msg.chat_id_) then
    ...
    send(msg.chat_id_, msg.id_, 1, '☑┊تم اضافه المجموعه الى الوضع المدفوع', 1, 'md')
     database:sadd('KENE:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  if   (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
  if not database:sismember('KENE:'..bot_id..'pro:groups',msg.chat_id_) then
  ...
  database:srem('KENE:'..bot_id..'pro:groups',msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, 'تم تعطيل ردود المطور', 1, 'md')
    database:set('KENE:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تفعيل ردود المدير', 1, 'md')
   database:del('KENE:'..bot_id..'repowner:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تعطيل ردود المدير', 1, 'md')
    database:set('KENE:'..bot_id..'repowner:mute'..msg.chat_id_,true)
  end
    end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تفعيل الايدي', 1, 'md')
   database:del('KENE:'..bot_id..'id:mute'..msg.chat_id_)
  end
  end
  ...
  send(msg.chat_id_, msg.id_, 1, '☑┊تم تعطيل الايدي', 1, 'md')
    database:set('KENE:'..bot_id..'id:mute'..msg.chat_id_,true)
  end
  end
  ...
  database:set("KENE"..bot_id..":sudo:gr",msg.chat_id_)
  end
  ...
  database:del("KENE"..bot_id..":sudo:gr")
  end
...
local all_photo_KENE = result.total_count_ 
local user_msgs = database:get('KENE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('KENE:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local edit = database:get('KENE:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local msguser = tonumber(database:get('KENE:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_))
if result.photos_[0] then
if is_sudo(msg) then
t = 'مطور البوت'
elseif is_creator(msg) then
t = 'منشئ الكروب'
elseif (database:get("KENE:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("KENE:all_if:"..database:get("KENE:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("KENE:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
...
if not database:get('KENE:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('KENE:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("KENE:gr:id:text:"..bot_id..msg.chat_id_) then 
local kali_text = {
...
kali3 = math.random(#kali_text)
 sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,"📷┊"..kali_text[kali3].."\n🌇┊صــورك ⊱  "..all_photo_KENE.."\n⚜┊ايديــك ⊱ ("..msg.sender_user_id_..")\n🎼┊مـعرفك ⊱ "..kali_info.."\n🎖┊موقعـك ⊱ "..t.."\n📊┊تفاعـلك ⊱  "..formsgg(msguser).."\n📨┊رسائلك ⊱  {"..(user_msgs + tahna).."}\n",msg.id_,msg.id_.."")
else
local KENE_new_text = database:get("KENE:gr:id:text:"..bot_id..msg.chat_id_)
local KENE_new_text = KENE_new_text:gsub('#username',(kali_info or 'لا يوجد'))
local KENE_new_text = KENE_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local KENE_new_text = KENE_new_text:gsub('#edit',(edit or 'لا يوجد'))
local KENE_new_text = KENE_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local KENE_new_text = KENE_new_text:gsub('#stast',(t or 'لا يوجد'))
local KENE_new_text = KENE_new_text:gsub('#photos',(all_photo_KENE or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,KENE_new_text,msg.id_,msg.id_.."")
end
else
if is_sudo(msg) then
t = 'مطور البوت'
...
local KENE_new_text = KENE_new_text:gsub('#stast',(t or 'لا يوجد'))   
local KENE_new_text = KENE_new_text:gsub('#photos',(all_photo_KENE or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, KENE_new_text, 1, 'html')
end   
end
else
end
...
getUser(msg.sender_user_id_, kali333)
  ...
  if  (text and text == 'تفعيل التثبيت') and is_owner(msg) then
   ...
   send(msg.chat_id_, msg.id_, 1, '☑┊تم تعطيل التثبيت', 1, 'md')
     database:set('KENE:'..bot_id..'pin:mute'..msg.chat_id_,true)
   end
     end
   ...
   pin(msg.chat_id_,msg.reply_to_message_id_,0)
       database:set('KENE:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
    ...
    database:set('KENE:'..bot_id..'viewget'..msg.sender_user_id_,true)
  ...
  redis:del('KENE:'..bot_id..'help', text)
  redis:del('KENE:'..bot_id..'h1', text)
  redis:del('KENE:'..bot_id..'h2', text)
  redis:del('KENE:'..bot_id..'h3', text)
  redis:del('KENE:'..bot_id..'h4', text)
  redis:del('KENE:'..bot_id..'h5', text)
  redis:del('KENE:'..bot_id..'h6', text)
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┊الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('KENE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('KENE:'..bot_id..'hhh'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENE:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
  redis:set('KENE:'..bot_id..'help', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   
-----------------------------
 if text == 'سمايلات' and database:get('KENE:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('KENE:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('KENE:'..bot_id..'klmos'..msg.chat_id_,name)
...
if text == ''..(database:get('KENE:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('KENE:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('KENE:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*📛¦ مبروك فزت \n📬¦ للعب مره اخره ارسل سمايلات*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('KENE:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)  

end
database:set('KENE:'..bot_id..'l:ids'..msg.chat_id_,true)
end 


if text == 'اسرع' and database:get('KENE:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('KENE:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('KENE:'..bot_id..'klmo'..msg.chat_id_,name)
...
if text == ''..(database:get('KENE:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('KENE:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('KENE:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*💎¦ مبروك فزت \n📬¦ للعب مره اخره ارسل الاسرع*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('KENE:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('KENE:'..bot_id..'l:id'..msg.chat_id_,true)
end 

if text == 'حزوره' and database:get('KENE:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('KENE:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('KENE:'..bot_id..'klmoa'..msg.chat_id_,name)
...
if text == ''..(database:get('KENE:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('KENE:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('KENE:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*💎¦ مبروك فزت \n📬¦ للعب مره اخره ارسل الاسرع*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('KENE:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('KENE:'..bot_id..'l:id1'..msg.chat_id_,true)
end 


if text =='مجوهراتي' then 
if tonumber((database:get('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(database:get('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(database:get('KENE:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
database:set('KENE:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
taha = (database:get('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('KENE:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('KENE:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('KENE:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('KENE:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:set('KENE:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
if text == 'تعطيل اللعبه' and is_owner(msg) then  
taha = '*📛¦*تم تعطيل اللعبه  ❌' 
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:del('KENE:'..bot_id..'lock_geam'..msg.chat_id_) 
end
if text == 'تفعيل اللعبه' and is_owner(msg) then  
...
-------------------------------------------------------------------------------------------------------------------   
   local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
    ...
    send(tostring((database:get("KENE"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بطرد البوت من المجموعه \n🎫┇ايدي الشخص ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف الشخص ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or database:get('KENE:'..bot_id.."group:link"..msg.chat_id_) or "لا يمكنني الوصول الى الرابط").."]" , 1, 'html')
    end
    tdcli_function ({
    ID = "GetChannelFull",
...
------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  local help = redis:get('KENE:'..bot_id..'help')
     local text =  
...
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎]]
 send(msg.chat_id_, msg.id_, 1, (help or text), 1, 'md')
  ...
  send(msg.chat_id_, msg.id_, 1, '📥┊الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
  redis:set('KENE:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
    return false end
  if text:match("^(.*)$") then
  local kali2 = redis:get('KENE:'..bot_id..'h11'..msg.sender_user_id_..'')
  if kali2 == 'msg' then
  send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
  redis:set('KENE:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
  redis:set('KENE:'..bot_id..'h1', text)
  send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
    return false end
   end
     if text:match("^م1$") and is_mod(msg) then
  local h1 = redis:get('KENE:'..bot_id..'h1')
     local text =  [[
📌⎮ 
...
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
  ...
  if (text and text == 'تغير امر م2') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┊الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('KENE:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kali2 = redis:get('KENE:'..bot_id..'h22'..msg.sender_user_id_..'')
   if kali2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('KENE:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
   redis:set('KENE:'..bot_id..'h2', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م5$") and is_mod(msg) then
  local res = redis:get('KENE:'..bot_id..'hres')
...
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
  ...
  send(msg.chat_id_, msg.id_, 1, (h2 or text), 1, 'md')
   ...
   local h2 = redis:get('KENE:'..bot_id..'h3')
     local text =  [[	 🥈⎮
🛠❯❯  اوامر الادمنيه  ❮❮🛠
  ﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
🌀❯ كتم/الغاء كتم »
...
🔸❯ مسح + قائمه المنع + الرابط+ المحضورين + البوتات + المكتومين +القوانين + الصوره  »
 ...
 📡⎮Ch ~⪼ @KENETEAM
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
   send(msg.chat_id_, msg.id_, 1, '📥┊الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
   redis:set('KENE:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
     return false end
   if text:match("^(.*)$") then
   local kali2 = redis:get('KENE:'..bot_id..'h44'..msg.sender_user_id_..'')
   if kali2 == 'msg' then
   send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
   redis:set('KENE:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
   redis:set('KENE:'..bot_id..'h4', text)
   send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
     return false end
    end
  if text:match("^م3$") and is_mod(msg) then
  local h4 = redis:get('KENE:'..bot_id..'h3')
  local text =  [[🥇┊ 
 🛠❯❯  اوامر المدراء ❮❮🛠
  ﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
...
🔹❯ فتح/قفل » الدردشه
  ...
  if (text and text == 'تغير امر م4') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
    send(msg.chat_id_, msg.id_, 1, '📥┊الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
    redis:set('KENE:'..bot_id..'h55'..msg.sender_user_id_..'', 'msg')
  return false end
    if text:match("^(.*)$") then
    local kali2 = redis:get('KENE:'..bot_id..'h55'..msg.sender_user_id_..'')
    if kali2 == 'msg' then
    send(msg.chat_id_, msg.id_, 1, '☑┊تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
    redis:set('KENE:'..bot_id..'h55'..msg.sender_user_id_..'', 'no')
    redis:set('KENE:'..bot_id..'h5', text)
    send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
  return
   false
 end
  ...
  local h4 = redis:get('KENE:'..bot_id..'h4')
...
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎     
♻️┊كود الرن ~⪼ `./KENE/ts`
⚠️┊ كود الحذف ~⪼ `rm -rf *KENE`
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎                
📮┊[قناة السورس](t.me/KENETEAM)
﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎﹎
]]
...
local svuser = 'KENE:'..bot_id..'user:'..result.id_
if username then
...
if database:get('KENE:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('KENE:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┊اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┊نورت الكروب ؛ @{us}'
end
...
if database:get('KENE:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:KENE"..msg.chat_id_..bot_id) then
local pin_id = database:get('KENE:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_kali = download_to_file('https://api.telegram.org/file/bot'..token..'/'..kali6.result.file_path, 'files_KENE/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('KENE:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
